...
The gate does not stop anyone from building the firmware: a public fork runs
the same workflow on free runners, and its owner can add an upload step for
their own products or build locally, because building for oneself is not
distribution. That is the recommended route for testers until the source
bundle below exists.
