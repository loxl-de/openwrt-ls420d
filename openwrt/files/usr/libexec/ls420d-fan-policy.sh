...
fan_disk_asleep() {
    # Query power mode before temperature I/O. Skip sleeping disks without
    # claiming a measured temperature; CPU and PHY protection remain active.
    local h="$1" b state
    for b in "$h"/device/block/*; do
        [ -e "$b" ] || return 1
        state="$(hdparm -C "/dev/${b##*/}" 2>/dev/null)" || return 1
        state="$(printf '%s\n' "$state" | sed -n 's/^[[:space:]]*drive state is:[[:space:]]*//p')"
        case "$state" in standby|sleeping) return 0;; esac
        return 1
    done
    return 1
}
